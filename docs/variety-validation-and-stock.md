...
- Controller (VarietyController@store, @update):
    - commodity_id: required|exists
    - name: required|string|max:255
    - description: nullable|string
    - minimum_limit: nullable|integer|min:0
    - status: nullable|in:available,out_of_stock,discontinued
    - image: required on create, nullable on update (maks 10MB)
    - **CATATAN:** Kolom price, stock, stock_bs_kg, stock_fs_kg sudah TIDAK divalidasi di level Variety. Harga dan stok dikelola di level Seed Lot.
    - Normalisasi: nilai null untuk minimum_limit diset menjadi 0 sebelum disimpan/diupdate.
...
- Create/Edit Views:
    ...
    - Validasi upload gambar diselaraskan ke maksimal 10MB.
    - Create Variety menggunakan pre-upload asinkron: gambar diunggah ke `storage/app/public/tmp/varieties` dan form menyimpan `temp_image_path`.
    - Guardian Guard: tombol Save tidak masuk loading bila gambar belum siap/unggahan belum selesai.
    - Immortality State: jika submit sudah masuk loading, watchdog memaksa tombol pulih otomatis (timeout/error JS).
